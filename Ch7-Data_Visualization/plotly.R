...
######load the library and make a basic plot

library(plotly)

plot_ly(data = Orange, x = ~age, y = ~circumference)
#######map the color and size of markers and hover over text to data
plot_ly(data = Orange, x = ~age, y = ~ circumference,
        color = ~Tree, size = ~age,
        text = ~paste("Tree ID: ", Tree, "<br>Age: ", age, "Circ: ", circumference)
)

#####add a second series/trace
...
##########add a drop-down menu so that you can select the plot type
